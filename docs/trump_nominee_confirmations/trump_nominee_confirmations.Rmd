...
author: "Jeff Lewis"
description: Putting Democratic senators' support for Trump appointees in recent historical context.
...
rc <- read_csv("https://voteview.com/static/data/out/rollcalls/Sall_rollcalls.csv",
...
overall_party_support <- dat |>
  ...
  summarize( pct_support = sum(cast_code %in% 1:3)/sum(cast_code %in% 1:6), .by = c(congress, party_code) ) |>
  pivot_wider(names_from = party_code, values_from = pct_support) |>
...
add_pres_boxes <- function(gg) {
   gg + 
    geom_rect(aes(xmin=100.66, xmax=102.33, ymin=30, ymax=105), 
                  fill="#FFF0F0", alpha=0.1, inherit.aes = FALSE) +
    geom_rect(aes(xmin=102.66, xmax=106.33, ymin=30, ymax=105), 
                  fill="#F2F2FF", alpha=0.1, inherit.aes = FALSE) +
    geom_rect(aes(xmin=106.66, xmax=110.33, ymin=30, ymax=105), 
                  fill="#FFF2F2", alpha=0.1, inherit.aes = FALSE) +
    geom_rect(aes(xmin=110.66, xmax=114.33, ymin=30, ymax=105), 
                  fill="#F2F2FF", alpha=0.1, inherit.aes = FALSE) +
    geom_rect(aes(xmin=114.66, xmax=116.33, ymin=30, ymax=105), 
...
overall_party_support_long <- overall_party_support |>
                        pivot_longer(cols = c(`100`, `200`), names_to = "party", values_to = "support") |>
...
party_supported <- dat |>
  filter(party_code %in% c(100,200),
         vote_question == "On the Nomination",
         !str_detect(str_to_lower(vote_desc), "judge|justice")) |>
  summarize( supported = sum(cast_code %in% 1:3)/sum(cast_code %in% 1:6) > 0.5, .by = c(congress, party_code, rollnumber)) |>
  summarize(supported = mean(supported), .by = c(congress, party_code)) |>
  pivot_wider(names_from = party_code, values_from = supported) |>
  mutate(pres = c("G. H. W. Bush", "G. H. W. Bush",
                  "Clinton", "Clinton", "Clinton", "Clinton",
                  "G. W. Bush", "G. W. Bush","G. W. Bush", "G. W. Bush",
                  "Obama", "Obama", "Obama", "Obama",
...
party_supported_long <- party_supported |>
                        pivot_longer(cols = c(`100`, `200`), names_to = "party", values_to = "supported") |>
                        mutate( party = ifelse(party=="100", "D", "R"),
                                of_pres_party = ifelse(party==pres_party,
                                                       "  President's party",
                                                       "  Opposition party"))
...
ggplot(party_supported_long, aes(x=congress, y=100*supported,
  ...
  add_pres_boxes() +
  geom_point(show.legend = FALSE) +
  geom_line(linewidth=0.15, color="grey")  +
  ylab("Percent of nominees supported") + xlab("Congress") +
  scale_x_continuous(breaks=seq(101, 116, 2), limits=c(100.5,118.35)) +
