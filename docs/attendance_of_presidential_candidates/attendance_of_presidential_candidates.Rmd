...
dat_by_month <- dat |>
                ...
                summarize(attend = sum(cast_code < 8)/n(),
                          n  = n(),
                          .by = c(bioname, icpsr, date, congress, cands, oc, rc)) |>
...
ggplot(dat_by_month |>
  ...
  geom_line(data=dat_by_month |>
              ...
              summarize(attend = mean(attend), .by = date),
            color=color[1], linewidth=0.5, alpha=0.4) +
  geom_line(data=dat_by_month |>
              filter(date > as.Date("2018-05-01"), oc,
                     date <= Sys.Date()) |>
              summarize(attend = mean(attend), .by = date),
            color=color[3], linewidth=0.5, alpha=0.4) +
  geom_line(data=dat_by_month |>
              filter(date > as.Date("2018-05-01"), rc,
                     date <= Sys.Date()) |>
              summarize(attend = mean(attend), .by = date),
            color=color[2], linewidth=0.5, alpha=0.4) +
  geom_line() +
  geom_point() +
  facet_wrap(~bioname, ncol=3) +
  theme_minimal() +
  xlab("Month") + ylab("Attendance (Share of Roll Calls Cast)") +
  ylim(0,100) +
