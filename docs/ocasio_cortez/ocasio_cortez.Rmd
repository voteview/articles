...
h116frosh_ids <- h_all |>
                   summarize(first_cong = min(congress), .by = icpsr) |>
...
# Calculate probabilities using nomprob
pp <- nomprob(yea = cbind(aoc_votes_prep$yea_1, aoc_votes_prep$yea_2),
              nay = cbind(aoc_votes_prep$no_1, aoc_votes_prep$no_2),
              ideal = places,
              Beta=8.8633,
              dimweight=c(1, 0.4616^2),
              normal=1)
prob_matrix <- t((aoc_votes_prep$cast_code==1)*t(pp) + (aoc_votes_prep$cast_code==6)*(1-t(pp)))

aoc_vts <- as.data.frame(prob_matrix) |>
  mutate(x1=places[,1], x2=places[,2]) |>
  pivot_longer(cols = -c(x1, x2), names_to = "vote", values_to = "prob") |>
...
dat <- h116_votes |>
         ...
         pivot_wider(names_from = rollnumber, values_from = cast_code, values_fill = 0) |>
         arrange(icpsr)
