...
Here we construct simple Party Loyalty Scores for members of the House and Senate.   These data provide in tabular form the rates of voting with their party majority for each member. Members in parties with fewer than 10 members are dropped. 
...
party_loyalty_score <- vote |> 
  ...
  mutate(v = case_when(cast_code %in% 1:3 ~ 1, # add in paired and announced yeas
                       ...
                       .default = NA)) |>
  mutate(party_majority = majority(v), .by = c(chamber, congress, rollnumber, party_code)) |>
  summarize(party_loyalty = 100*mean(v == party_majority, na.rm=TRUE),
            no_of_votes = sum(!is.na(v)),
           .by = c(congress, chamber, icpsr, bioname, state_abbrev, district_code, party_code)) |>
  mutate(floor_median_party_loyalty = median(party_loyalty, na.rm = TRUE), .by = c(congress, chamber)) |>
  mutate(caucus_median_party_loyalty = median(party_loyalty, na.rm = TRUE),
         n_party = n(), .by = c(congress, chamber, party_code)) |>
  filter(n_party > 9)
```  
