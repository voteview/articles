...
Here we describe how we create and continue the Party Unity Scores that Keith Poole previously posted [here](https://legacy.voteview.com/Party_Unity.htm).   We produce both member- and Congress-level unity data.  We also include a figures showing the change in the scores over time and compare these new scores to the ones previously produced by Poole.

You can download a Congress-chamber party unity summary [here](party_unity_by_congress.csv) and a member level party unity scores [here](party_unity_by_member.csv).
...
big_party <- member_dat |>
                ...
                summarize(num=n(), .groups="drop", .by=c(congress, chamber, party_code)) |>
                slice_max(num, n=2, by=c(congress, chamber)) |>
                select(-num)
```
...
unity_votes <- major_dat |>
                  ...
                  filter(cast_code != 0) |>
                  summarize(n=n(), .groups="drop", .by=c(congress, chamber, rollnumber, party_code, cast_code)) |>
                  filter(n()==1 | n>min(n), .by=c(congress, chamber, rollnumber, party_code)) |>
                  summarize(unity = min(cast_code) != max(cast_code), .groups="drop", .by=c(congress, chamber, rollnumber))

unity_votes_by_congress <- unity_votes |>
                  summarize(unity_votes = sum(unity),
                            total_votes = n(),
                            .groups = "drop",
                            .by = c(chamber, congress)) |>
                  mutate(percent_unity = unity_votes/total_votes) |>
                  pivot_longer(cols = c(unity_votes, total_votes, percent_unity),
                               names_to = "fld", values_to = "val") |>
                  unite(chamber_fld, chamber, fld) |>
                  pivot_wider(names_from = chamber_fld, values_from = val) |>
                  rename_with(tolower)
write_csv(unity_votes_by_congress, file="party_unity_by_congress.csv")
...
ubc <- bind_rows(unity_votes_by_congress |> mutate(version="New"),
       ...
       pivot_longer(cols = contains("percent"), names_to = "chamber", values_to = "percent") |>
...
We limit the votes to just those that are party unity votes as determined above. We filter out the voters from prior to the 35th Congress as well as all of the abstentions and missed votes.For every congress, chamber, roll call and party, we determine the modal vote.  We then count the number of times that each member voted with the modal member of their party across party unity votes.   
...
party_unity_members <- major_dat |>
          ...
          mutate(cast_code = vote_code_map[cast_code]) |>
          filter(cast_code > 0) |>
          mutate(party_maj_choice = median(cast_code), .by = c(congress, chamber, rollnumber, party_code)) |>
          summarize(total_unity_votes=n(),
                    with_party_unity_votes = sum(cast_code==party_maj_choice),
                    .groups="drop",
                    .by = c(congress, chamber, icpsr, bioname, party_code, state_abbrev, district_code)) |>
          mutate(party_unity_score = with_party_unity_votes/total_unity_votes )

write_csv(party_unity_members, file="party_unity_by_member.csv")
rm(major_dat)
```
